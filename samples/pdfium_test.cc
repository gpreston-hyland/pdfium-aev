...
struct Options {
  ...
  std::vector<int> pagesArray;
  time_t time = -1;
};

int PageRenderFlagsFromOptions(const Options& options) {
  int flags = FPDF_ANNOT_FLAG_NONE;
  if (options.lcd_text)
    flags |= FPDF_LCD_TEXT;
...
bool ParseCommandLine(const std::vector<std::string>& args,
  ...
  for (; cur_idx < args.size(); ++cur_idx) {
    ...
    if (cur_arg == "--show-config") {
    ...
    } else if (cur_arg == "--thumbnails") {
      options->thumbnails = true;
    } else if (ParseSwitchKeyValue(cur_arg, "--uniqueId=", &value)) {
      ...
      options->pages = true;
      std::string pages_string = value;
      size_t first_dash = pages_string.find("-");
      size_t first_comma = pages_string.find(",");
      if (first_dash != std::string::npos && first_comma != std::string::npos) {
        fprintf(stderr, "The --pages option has to be either consecutive pages like '1-7', a single page, or comma seperated pages '1,3,5,7'. It can not handle both\n");
        return false;
      } else if (first_dash != std::string::npos) {
        int start = std::stoi(pages_string.substr(0, first_dash));
        int end = std::stoi(pages_string.substr(first_dash + 1));
        for (int i = start; i <= end; i++) {
          options->pagesArray.push_back(i);
        }
      } else if (first_comma != std::string::npos) {
        //Split the string for commas
        std::string token;
        size_t pos = 0;
        while ((pos = pages_string.find(',')) != std::string::npos) {
          token = pages_string.substr(0, pos);
          options->pagesArray.push_back(std::stoi(token));
          pages_string.erase(0, pos + 1);
        } 
        //need to add the final page
        std::cout << pages_string << std::endl;
        options->pagesArray.push_back(std::stoi(pages_string));
      } else {
        options->pagesArray.push_back(std::stoi(pages_string));
      }
    } else if (cur_arg == "--md5") {
    ...
    switch (options.output_format) {
      ...
      case OutputFormat::kPng:
        if (options.thumbnails) {
          image_file_name = WritePng(name.c_str(), page_index, buffer, stride,
                                     width, height, true, options.uniqueId);
        } else {
          image_file_name = WritePng(name.c_str(), page_index, buffer, stride,
                                     width, height, false, options.uniqueId);
        }
...
void ProcessPdf(const std::string& name,
 ...
 // int page_count = FPDF_GetPageCount(doc.get());
  ...
  int bad_pages = 0;
  for (auto i = options.pagesArray.begin(); i != options.pagesArray.end(); ++i) {
    if (is_linearized) {
      int avail_status = PDF_DATA_NOTAVAIL;
      while (avail_status == PDF_DATA_NOTAVAIL)
        avail_status =
            FPDFAvail_IsPageAvail(pdf_avail.get(), *i, &hints);
    ...
    if (ProcessPage(name, doc.get(), form.get(), &form_callbacks, *i,
                    options,
                    events)) {
      ++processed_pages;
    } else {
      fprintf(stderr, "Could not process page %d for some reason\n", *i);
      ++bad_pages;
    }
  }
...
constexpr char kUsageString[] =
    ...
    "  --pages=<number>       - only render the single page\n"
    "  --pages=<number>,<number> - render pages (comma seperated)"
#ifdef _WIN32
