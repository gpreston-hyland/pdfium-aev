...
std::string WritePng(const char* pdf_name,
  ...
  int chars_formatted;
  if (thumbnail) {
    chars_formatted = snprintf(filename, sizeof(filename), "%s.%d.%s.thumbnail.png", pdf_name,
                 num, uniqueId.c_str());
  } else {
    chars_formatted = snprintf(filename, sizeof(filename), "%s.%d.%s.png",
                               pdf_name, num, uniqueId.c_str());
  }
  if (chars_formatted < 0 ||
      static_cast<size_t>(chars_formatted) >= sizeof(filename)) {
    fprintf(stderr, "Filename %s is too long\n", filename);
